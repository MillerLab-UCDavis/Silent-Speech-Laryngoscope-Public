...
def load_audio(filename, start=None, end=None, max_frames=None, renormalize_volume=False):
    ...
    if r == 22050:
        # print(audio.shape[0]/22050.0)
        audio = librosa.resample(audio, orig_sr=22050, target_sr=16000)
        # print(audio.shape[0]/16000.0)
    else:
        # print(audio.shape[0]/16000.0)
        assert r == 16000
    audio = np.clip(audio, -1, 1) # because resampling sometimes pushes things out of range
    pytorch_mspec = mel_spectrogram(torch.tensor(audio, dtype=torch.float32).unsqueeze(0), 1024, 80, 16000, 256, 1024, 0, 8000, center=False)
...
# def combine_fixed_length(tensor_list, length, lengths=None):
...
#         len = t.size(0)
#         curr_batch_len = math.ceil(len/length)
#         # curr_batch_size = round(len/length, 2)
#         # if prev_sizes is not None:
#         #     print("Curr size: ",len/length, "Audio size: ", prev_sizes[i])
        
#         if(lengths is not None):
#             if(curr_batch_len <= lengths[i]):
#                 pad_length = lengths[i]*length-len
#                 zeros_to_add = torch.zeros(pad_length, *t.size()[1:], dtype=t.dtype, device=t.device)
#                 tensor_list[i] = torch.cat((t, zeros_to_add), dim=0)
#                 total_length += pad_length
#                 batch_lengths.append(lengths[i])
#             elif(curr_batch_len > lengths[i]):
#                 # print("SEE Here: ", curr_batch_len, len)
#                 new_length = lengths[i]*length
#                 # zeros_to_add = torch.zeros(pad_length, *t.size()[1:], dtype=t.dtype, device=t.device)
#                 tensor_list[i] = t[:new_length]
#                 total_length -= (len-new_length)
#                 batch_lengths.append(lengths[i])
#             assert batch_lengths[i] == lengths[i]
#         else:
#             if len % length != 0:
#                 pad_length = length - (len % length)
#                 zeros_to_add = torch.zeros(pad_length, *t.size()[1:], dtype=t.dtype, device=t.device)
#                 # print("Old: ", t.shape)
#                 tensor_list[i] = torch.cat((t, zeros_to_add), dim=0)
#                 # print("New: ", tensor_list[i].shape)
#                 total_length += pad_length
#                 batch_lengths.append(math.ceil(len / length))
#             else:
#                 batch_lengths.append(len // length)
...
#     return tensor.view(n, length, *tensor.size()[1:]), batch_lengths
     



#     # to make audio equal to emg
# def combine_fixed_length(tensor_list, length, lengths=None):
#     # for i in range(len(tensor_list)):
#     #     print(tensor_list[i].shape)
#     # print(len(tensor_list))
...
#         len = t.size(0)
#         curr_batch_len = math.ceil(len/length)
#         assert lengths == None or curr_batch_len >= lengths[i]
#         if(lengths != None and curr_batch_len > lengths[i]):
#             tensor_list[i] = t[:lengths[i]*length]
#             batch_lengths.append(lengths[i])
#         elif len % length != 0:
#             pad_length = length - (len % length)
#             zeros_to_add = torch.zeros(pad_length, *t.size()[1:], dtype=t.dtype, device=t.device)
...
#             # print("New: ", tensor_list[i].shape)
#             total_length += pad_length
#             batch_lengths.append((len // length)+1)
#         else:
#             batch_lengths.append(len // length)
...
def combine_fixed_length_for_training(tensor_list, length):
    # for i in range(len(tensor_list)):
    #     print(tensor_list[i].shape)
    # print(len(tensor_list))
...
def combine_fixed_length(tensor_list):
    # Find the maximum length among all tensors in the list
    max_length = max(len(tensor) for tensor in tensor_list)
    # print(max_length)

    # Create an empty list to store padded tensors
    padded_tensors = []
    masks = []
    
    # Iterate through the input tensor list
    for tensor in tensor_list:
        # Calculate the padding size for the current tensor
        padding_size = max_length - len(tensor)
        # print(padding_size)
        
        # Pad the tensor with zeros along the first dimension
        padding = (0, 0) * (tensor.dim() - 1) + (0, padding_size)
        padded_tensor = torch.nn.functional.pad(tensor, padding)

        # Create a mask for the padded values
        mask = torch.ones_like(padded_tensor)
        if(padding_size != 0):
            mask[-padding_size:] = 0  # Set zeros for padded values
    ...
    # print(tensor_list[0], padded_tensors[0])
    ...
    # print(padded_tensors[0], masks[0])

    return combined_tensor, combined_mask

def decollate_tensor(seq, lengths):
    # s, d = tensor.size()
    # tensor = tensor.view(s*d)
    results = []
    idx = 0
    for length in lengths:
        combined_list = []
        for _ in range(length):
            combined_list.extend(seq[idx])
            idx += 1
