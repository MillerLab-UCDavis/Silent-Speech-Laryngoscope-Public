...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, n_head, drop_prob, dim_feedforward=2048):
        super(TransformerDecoderLayer, self).__init__()
        self.self_attention = MultiHeadAttention(d_model=d_model, n_head=n_head, dropout=drop_prob, relative_positional=True, relative_positional_distance=100)
        self.norm1 = nn.LayerNorm(d_model)
        self.dropout1 = nn.Dropout(p=drop_prob)

        self.enc_dec_attention = MultiHeadAttention(d_model=d_model, n_head=n_head, dropout=drop_prob, relative_positional=False)
        self.norm2 = nn.LayerNorm(d_model)
    ...
    def forward(self, dec, enc, tgt_mask, memory_mask, tgt_key_padding_mask=None, memory_key_padding_mask=None):
        # 1. compute self attention
        _x = dec
        # print(dec.shape, tgt_mask.shape)
        x = self.self_attention(dec, mask=tgt_mask)
        # 2. add and norm
        x = self.dropout1(x)
        x = self.norm1(x + _x)
        ...
        if enc is not None:
            ...
            # print("y shape ", enc.shape)
            x = self.enc_dec_attention(x, y=enc, mask=memory_mask)
...
class MultiHeadAttention(nn.Module):
  ...
  def forward(self, x, y=None, mask=None):
    ...
    q = torch.einsum('tbf,hfa->bhta', x, self.w_q)  ## time*bs*d_model    n_head*d_model*d_qkv   => bs*n_head*time*d_qkv
    if(y == None):
        k = torch.einsum('tbf,hfa->bhta', x, self.w_k)
    else:
        k = torch.einsum('tbf,hfa->bhta', y, self.w_k)

    if(y == None):
        v = torch.einsum('tbf,hfa->bhta', x, self.w_v)
    else:
        v = torch.einsum('tbf,hfa->bhta', y, self.w_v)

    if(y == None):
        if(mask is not None and len(mask.shape) < 4):
            _, _, time = mask.shape
            mask = mask.unsqueeze(2).expand(-1, -1, time, -1)
    else:
        if(mask is not None and len(mask.shape) < 4):
            time, _, _ = x.shape
            mask = mask.unsqueeze(2).expand(-1, -1, time, -1)
    ...
    if mask is not None:
        logits = logits.masked_fill((mask == 0) | (mask == False), -1e4)  # Replace masked positions


    if self.relative_positional is not None:
        q_pos = q.permute(2,0,1,3) #bhqd->qbhd   time*bs*n_head*d_qkv
        l,b,h,d = q_pos.size()
        position_logits, _ = self.relative_positional(q_pos.reshape(l,b*h,d))
        logits = logits + position_logits.view(b,h,l,l) ## bs*n_head*time*time
