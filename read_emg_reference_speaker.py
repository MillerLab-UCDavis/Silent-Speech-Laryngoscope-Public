...
FLAGS = flags.FLAGS
flags.DEFINE_list('remove_channels', [2, 3, 5, 8, 9, 10, 12, 14, 16, 17, 18, 19], 'channels to remove') # [ 7, 11,  0,  6, 10, 13,  1,  4], [16, 13, 14, 8, 2, 3, 19], [0, 1, 5, 6, 7, 8, 10, 11, 13, 14, 16, 19]
flags.DEFINE_list('silent_data_directories', ['./reference_speaker_data/closed_vocab/silent'], 'silent data locations')
flags.DEFINE_list('voiced_data_directories', ['./reference_speaker_data/closed_vocab/voiced'], 'voiced data locations')
...
#         frame = Image.open(frame_path).convert('RGB')  # Convert image to RGB
#         frames.append(np.array(frame))  # Convert PIL image to numpy array
    
#     frames = np.stack(frames, axis=0)  # Convert list of arrays to 4D array (T, H, W, C)
#     return frames
    ...
    def __iter__(self):
        ...
        for i, idx in enumerate(indices):
            ...
            if not np.any([l in string.ascii_letters for l in info]):
                continue
            length = np.load(os.path.join(directory_info.directory, f'{file_idx}_emg.npy')).shape[0]
            # print("emg length : ", length)
            # print("Size aware sampler ",length)
...
class EMGDataset(torch.utils.data.Dataset):
    def __init__(self, base_dir=None, limit_length=False, dev=False, test=False, no_testset=False, no_normalizers=False):
        ...
        for directory_info in directories:
            ...
            for fname in os.listdir(directory_info.directory):
                ...
                if m is not None:
                    ...
                    if directory_info.silent and ((test and location_in_testset and not directory_info.exclude_from_testset) \
                    ...
                    if not directory_info.silent:
                        voiced = (directory_info, trial)
                        self.voiced_data_locations[(sess, trial)] = voiced
        ...
        self.my_augmenter = AddNoise(scale=0.1)
    ...
    def __getitem__(self, i):
        ...
        if directory_info.silent:
            ...
            audio, emg, text, phonemes, video_frames = load_utterance(directory_info.directory, idx, self.limit_length, text_align_directory=self.text_align_directory, is_silent=voiced_directory.directory, augmenter=self.my_augmenter)
            
        else:
            audio, emg, text, phonemes, video_frames = load_utterance(directory_info.directory, idx, self.limit_length, text_align_directory=self.text_align_directory, is_silent=None, augmenter=self.my_augmenter)
