...
class ctcBeamSearch(object):
    ...
    def decode(self, inputs, inputs_list):
        ...
        for batch in range(batches):
            mat = inputs[batch].detach().cpu().numpy()
            # print("size of mat is ", mat.shape)
            ...
            for t in range(inputs_list[batch]):
                curr=BeamState()
                #跳过概率很接近1的blank帧，增加解码速度 Skip blank frames with a probability close to 1 to increase decoding speed.
                if (1 - mat[t, self.blank_index]) < 0.1:
                    continue
                #取前beam个最好的结果  Take the top beam results.
                ...
                for y in BHat:
                    ...
                    if len(y)>0:
                        ...
                        #There are two possibilities for the same 'y': either add a duplicate or add a 
                        #blank. If there were no characters before, the NonBlank probability is 0
                    ...
                    #At time t, when adding another label, the probability of a blank is 0. 
                    #If the added label is the same as the last one, it must be preceded by a blank 
                    #because repetition is not allowed.
                    for k in range(maxC):
...
class Decoder(object):
    ...
    def __init__(self, blank_index = 100, remove_rep=True, silent = None):
        self.blank_index = blank_index
        self.silent = silent
...
class BeamDecoder(Decoder):
    ...
    def __init__(self, int2char, beam_width = 20, blank_index = 100, lm_path=None, lm_alpha=0.01):
        self.beam_width = beam_width
        super(BeamDecoder, self).__init__(int2char, blank_index)
