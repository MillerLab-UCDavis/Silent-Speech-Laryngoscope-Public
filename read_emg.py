...
flags.DEFINE_string('testset_file', 'testset_closed_vocab.json', 'file with testset indices')
...
def load_utterance(base_dir, index, limit_length=False, debug=False, text_align_directory=None):
    ...
    return audio, emg_true, mfccs, emg_features, info['text'], (info['book'],info['sentence_index']), phonemes, emg_orig.astype(np.float32)
...
class EMGDataset(torch.utils.data.Dataset):
    def __init__(self, base_dir=None, limit_length=False, dev=False, test=False, no_testset=False, no_normalizers=False):
        ...
        # print("HELLOOOOOO", len(self.example_indices))
        ...
        audio, emg_true, sample_mfccs, sample_emg, _, _, _, _ = load_utterance(self.example_indices[0][0].directory, self.example_indices[0][1])
    ...
    def __getitem__(self, i):
        ...
        audio, emg_true, mfccs, emg, text, book_location, phonemes, raw_emg = load_utterance(directory_info.directory, idx, self.limit_length, text_align_directory=self.text_align_directory)
        # if not self.test:
        #     emg_true = self.spec_augment(torch.tensor(emg_true)).numpy()
        raw_emg = raw_emg / 20
        raw_emg = 50*np.tanh(raw_emg/50.)

        # emg_true = emg_true / 20
        # emg_true = 50*np.tanh(emg_true/50.)

        # units = np.load(directory_info.directory+"/"+str(idx)+"_units.npy")
        ...
        if directory_info.silent:
            ...
            audio, voiced_emg_true, voiced_mfccs, voiced_emg, _, _, phonemes, _ = load_utterance(voiced_directory.directory, voiced_idx, False, text_align_directory=self.text_align_directory)
            # if not self.test:
            #     emg_true = self.spec_augment(torch.tensor(emg_true)).numpy()
        ...
        result['emg_true'] = torch.from_numpy(emg_true).pin_memory()
        # result['units'] = torch.from_numpy(units).pin_memory()
    ...
    def collate_raw(batch):
        ...
        # max_emg_length = max([ex['emg_true'].shape[0] for ex in batch])  # Find the maximum length

        # emg_padded = []
        # masks = []
        # for ex in batch:
        #     emg = ex['emg_true']
        #     padding_length = max_emg_length - emg.shape[0]
        #     padded_emg = np.pad(emg, ((0, padding_length), (0, 0)), mode='constant')  # Pad with zeros
        #     emg_padded.append(padded_emg)

        #     mask = np.ones_like(emg, dtype=np.float32)  # Create a mask with ones
        #     mask = np.pad(mask, ((0, padding_length), (0, 0)), mode='constant')  # Pad the mask
        #     masks.append(mask)
        ...
        # units = [ex['units'] for ex in batch]
        emg_true = [ex['emg_true'] for ex in batch]
